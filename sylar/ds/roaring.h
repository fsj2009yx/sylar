...
static inline void *get_writable_copy_if_shared(void *candidate_shared_container, uint8_t *type) {
    if (*type == SHARED_CONTAINER_TYPE_CODE) {
        return shared_container_extract_copy((shared_container_t *)candidate_shared_container,
                                             type);
    } else {
        return candidate_shared_container;
    }
...
#else
            bitset_container_or_nocard((const bitset_container_t *)c1,
                                       (const bitset_container_t *)c2, (bitset_container_t *)c1);
...
/**
 * Move the iterator to the first value >= val. If there is a such a value, then it->has_value is
 * true. The new value is in it->current_value. For convenience, returns it->has_value.
 */
