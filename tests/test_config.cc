...
int main(int argc, char** argv) {
    ...
    test_loadconf();
    std::cout << g_tconf->toString() << std::endl;
    // auto tmp = sylar::Config::Lookup<TConf, sylar::PackDecodeCast<TConf>,
    // sylar::PackEncodeCast<TConf>>("tconf"); std::cout << tmp->toString() << std::endl; std::cout
    // << SYLAR_GET_CONFIG(TConf, "tconf")->toString() << std::endl;
    return 0;
