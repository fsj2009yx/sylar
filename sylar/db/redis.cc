...
static sylar::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_redis =
    sylar::Config::Lookup("redis.config",
    ...
    if (c) {
        ...
        if (!m_passwd.empty()) {
            ...
            if (r->type != REDIS_REPLY_STATUS) {
                SYLAR_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "(" << m_host
                                          << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if (!r->str) {
                SYLAR_LOG_ERROR(g_logger) << "auth reply str error: NULL(" << m_host << ":"
                                          << m_port << ", " << m_name << ")";
                return false;
            }
            if (strcmp(r->str, "OK") == 0) {
                ...
                SYLAR_LOG_ERROR(g_logger) << "auth error: " << r->str << "(" << m_host << ":"
                                          << m_port << ", " << m_name << ")";
                return false;
            }
        }
...
ReplyPtr Redis::cmd(const char* fmt, va_list ap) {
    ...
    if (!r) {
        if (m_logEnable) {
            SYLAR_LOG_ERROR(g_logger) << "redisCommand error: (" << fmt << ")(" << m_host << ":"
                                      << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
...
ReplyPtr Redis::cmd(const std::vector<std::string>& argv) {
    ...
    if (m_logEnable) {
        SYLAR_LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host << ":" << m_port << ")("
                                  << m_name << ")" << r->str;
    }
    return nullptr;
}
...
ReplyPtr Redis::getReply() {
    ...
    if (m_logEnable) {
        SYLAR_LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host << ":" << m_port << ")("
                                  << m_name << ")";
    }
    return nullptr;
}
...
void FoxRedis::OnAuthCb(redisAsyncContext* c, void* rp, void* priv) {
    ...
    if (!r) {
        SYLAR_LOG_ERROR(g_logger) << "auth error:(" << fr->m_host << ":" << fr->m_port << ", "
                                  << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
        SYLAR_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "(" << fr->m_host << ":"
                                  << fr->m_port << ", " << fr->m_name << ")";
        return;
    }
    if (!r->str) {
    ...
    if (strcmp(r->str, "OK") == 0) {
        SYLAR_LOG_INFO(g_logger) << "auth ok: " << r->str << "(" << fr->m_host << ":" << fr->m_port
                                 << ", " << fr->m_name << ")";
    } else {
        SYLAR_LOG_ERROR(g_logger) << "auth error: " << r->str << "(" << fr->m_host << ":"
                                  << fr->m_port << ", " << fr->m_name << ")";
    }
}
...
void FoxRedis::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if (!status) {
        SYLAR_LOG_INFO(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                                 << " success";
        ...
        SYLAR_LOG_ERROR(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                                  << " fail, error:" << c->errstr;
...
void FoxRedis::DisconnectCb(const redisAsyncContext* c, int status) {
    SYLAR_LOG_INFO(g_logger) << "FoxRedis::DisconnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                             << " status:" << status;
    FoxRedis* ar = static_cast<FoxRedis*>(c->data);
    ar->m_status = UNCONNECTED;
}
...
bool FoxRedis::pinit() {
    ...
    if (!ctx) {
        SYLAR_LOG_ERROR(g_logger) << "redisAsyncConnect (" << m_host << ":" << m_port << ") null";
        return false;
    }
    if (ctx->err) {
...
void FoxRedisCluster::OnAuthCb(redisClusterAsyncContext* c, void* rp, void* priv) {
    ...
    redisReply* r = (redisReply*)rp;
    if (!r) {
        SYLAR_LOG_ERROR(g_logger) << "auth error:(" << fr->m_host << ", " << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
        SYLAR_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "(" << fr->m_host
    ...
    if (strcmp(r->str, "OK") == 0) {
        SYLAR_LOG_INFO(g_logger) << "auth ok: " << r->str << "(" << fr->m_host << ", " << fr->m_name
                                 << ")";
    } else {
        SYLAR_LOG_ERROR(g_logger) << "auth error: " << r->str << "(" << fr->m_host << ", "
                                  << fr->m_name << ")";
...
void FoxRedisCluster::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if (!status) {
        SYLAR_LOG_INFO(g_logger) << "FoxRedisCluster::ConnectCb " << c->c.tcp.host << ":"
                                 << c->c.tcp.port << " success";
    } else {
        SYLAR_LOG_ERROR(g_logger) << "FoxRedisCluster::ConnectCb " << c->c.tcp.host << ":"
                                  << c->c.tcp.port << " fail, error:" << c->errstr;
    }
}

void FoxRedisCluster::DisconnectCb(const redisAsyncContext* c, int status) {
    SYLAR_LOG_INFO(g_logger) << "FoxRedisCluster::DisconnectCb " << c->c.tcp.host << ":"
...
void FoxRedisCluster::pcmd(FCtx* fctx) {
    if (m_status != CONNECTED) {
        SYLAR_LOG_INFO(g_logger) << "redis (" << m_host << ") unconnected " << fctx->cmd;
        init();
        if (fctx->fiber) {
            fctx->scheduler->schedule(&fctx->fiber);
        }
    ...
    for (auto& i : m_config) {
        ...
        for (int n = 0; n < pool; ++n) {
            if (type == "redis") {
                ...
                auto name = i.first;
                sylar::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done]() {
                    sylar::FoxRedisCluster* rds(
                        new sylar::FoxRedisCluster(sylar::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
