...
std::string GetConsulUniqID(int port) {
    return sylar::GetHostName() + "-" + sylar::replace(sylar::GetIPv4(), '.', '-') + "-" +
           std::to_string(port) + "-" + std::to_string((sylar::GetCurrentUS() % 100000) + 100000);
}
...
bool ConsulClient::serviceRegister(ConsulRegisterInfo::ptr info) {
    ...
    auto rt = sylar::http::HttpConnection::DoRequest(sylar::http::HttpMethod::PUT, url, 5000,
                                                     {{"content-type", "application/json"}}, data);
    if (rt->result) {
        SYLAR_LOG_ERROR(g_logger) << "register fail, " << url << " - " << data << " - "
                                  << rt->toString();
        return false;
    }
...
bool ConsulClient::serviceUnregister(const std::string& id) {
    for (int i = 0; i < 2; ++i) {
        std::string url =
            m_url + "/v1/agent/service/deregister/" + sylar::StringUtil::UrlEncode(id);
        auto rt = sylar::http::HttpConnection::DoRequest(
            sylar::http::HttpMethod::PUT, url, 5000, {{"content-type", "application/json"}}, "");
        if (rt->result) {
            SYLAR_LOG_ERROR(g_logger) << "unregister fail, " << url << " - " << rt->toString();
            return false;
        }
...
bool ConsulClient::serviceQuery(const std::unordered_set<std::string>& service_names,
    ...
    if (rt->result) {
        SYLAR_LOG_ERROR(g_logger) << "servicequery fail, " << url << " - " << rt->toString();
        return false;
    }
    ...
    if (!sylar::pack::DecodeFromJsonString(rt->response->getBody(), m, 0)) {
        SYLAR_LOG_ERROR(g_logger) << "servicequery parse fail," << url << " - "
                                  << rt->response->getBody();
        return false;
    }
    ...
    SYLAR_LOG_INFO(g_logger) << "serviceQuery " << sylar::pack::EncodeToJsonString(service_names, 0)
