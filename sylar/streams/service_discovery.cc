...
static sylar::ConfigVar<uint32_t>::ptr g_service_discover_redis_ttl = sylar::Config::Lookup(
...
static sylar::ConfigVar<uint32_t>::ptr g_service_discover_redis_check =
    sylar::Config::Lookup("service_discovery.redis.check_interval", (uint32_t)3,
                          "service discovery redis check interval");
static sylar::ConfigVar<uint32_t>::ptr g_service_discover_consul_check =
    sylar::Config::Lookup("service_discovery.consul.check_interval", (uint32_t)15,
...
bool ZKServiceDiscovery::registerInfo(const std::string& domain, const std::string& service,
    ...
    if (!m_isOnTimer) {
        SYLAR_LOG_ERROR(g_logger) << "create path=" << (path + "/" + ip_and_port)
...
bool ZKServiceDiscovery::queryInfo(const std::string& domain, const std::string& service) {
    if (service != "all") {
        ...
        if (!m_isOnTimer) {
            SYLAR_LOG_ERROR(g_logger) << "create path=" << (path + "/" + m_selfInfo)
                                      << " fail, error:" << zerror(rt) << " (" << rt << ")";
        }
        return rt == ZNODEEXISTS;
    } else {
...
bool ZKServiceDiscovery::getChildren(const std::string& path) {
    ...
    if (!ParseDomainService(path, domain, service)) {
        SYLAR_LOG_ERROR(g_logger) << "get_children path=" << path << " invalid path";
        return false;
    }
    {
        sylar::RWMutex::ReadLock lock(m_mutex);
    ...
    if (v != ZOK) {
        SYLAR_LOG_ERROR(g_logger) << "get_children path=" << path << " fail, error:" << zerror(v)
                                  << " (" << v << ")";
        return false;
    }
    ...
    for (auto& i : vals) {
        ...
        SYLAR_LOG_INFO(g_logger) << "domain=" << domain << " service=" << service
                                 << " info=" << info->toString();
    }
...
void ZKServiceDiscovery::onWatch(int type, int stat, const std::string& path,
    ...
    SYLAR_LOG_ERROR(g_logger) << "onWatch hosts=" << m_hosts << " type=" << type << " stat=" << stat
    ...
    for (auto& i : rinfo) {
        ...
        for (auto& n : i.second) {
            ...
            for (auto& v : n.second) {
                ...
                if (!sylar::RedisUtil::TryCmd(m_name, 5, "hset sylar:%s:%s %s %s", i.first.c_str(),
                                              n.first.c_str(), v.first.c_str(),
                                              MapToStr(m).c_str())) {
                    SYLAR_LOG_ERROR(g_logger) << "register hset sylar:" << i.first << ":" << n.first
...
bool RedisServiceDiscovery::queryInfo() {
    ...
    for (auto& i : infos) {
        ...
        // SYLAR_LOG_INFO(g_logger) << "==" << i.first << " - " << sylar::Join(i.second.begin(),
        // i.second.end(), ",");
        ...
        for (auto& n : services) {
            ...
            if (!rpy) {
                SYLAR_LOG_ERROR(g_logger)
                    << "query_info error, hgetall sylar:" << i.first << ":" << n;
                continue;
            }
...
ConsulServiceDiscovery::ConsulServiceDiscovery(const std::string& name,
                                               sylar::ConsulClient::ptr client,
                                               sylar::ConsulRegisterInfo::ptr regInfo)
...
bool ConsulServiceDiscovery::queryInfo() {
    ...
    for (auto& i : infos) {
        ...
        if (!m_client->serviceQuery(i.second, cinfos)) {
            SYLAR_LOG_ERROR(g_logger) << "consul query service fail, "
                                      << sylar::Join(i.second.begin(), i.second.end(), ",");
            return false;
        }
        std::unordered_map<uint64_t, ServiceItemInfo::ptr> sinfos;
        for (auto& it : cinfos) {
