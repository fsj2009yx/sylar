...
static sylar::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_tair =
    sylar::Config::Lookup(
        "tair.config", std::map<std::string, std::map<std::string, std::string> >(), "tair config");

Tair::Tair() : m_client(nullptr), m_sn(0), m_timeout(2000), m_threadCount(4) {
...
const std::string Tair::toString() {
    std::stringstream ss;
    ss << "tair client: master_addr=" << m_masterAddr << " slave_addr=" << m_slaveAddr
       << " group_name=" << m_groupName << " thread_count=" << m_threadCount
       << " log_level=" << m_logLevel << " log_file=" << m_logFile << " timeout=" << m_timeout
       << " cache=[";
    bool is_first = true;
    sylar::Mutex::Lock lock(m_mutex);
    for (auto& i : m_caches) {
...
int32_t Tair::get(const std::string& key, std::string& val, int area, int timeout_ms) {
    ...
    if (timeout_ms) {
        c->timer =
            sylar::IOManager::GetThis()->addTimer(timeout_ms, std::bind(&Tair::onTimer, this, c));
    }
...
int32_t Tair::put(const std::string& key, const std::string& val, int area, int expired,
                  int version) {
    tair::common::data_entry dbkey(key.c_str(), key.size(), false);
...
void Tair::OnGetCb(int result, const tair::common::data_entry* key,
    ...
    if (!ctx) {
        if (result) {
            SYLAR_LOG_ERROR(g_logger)
                << "OnGetCb result=" << result << " key=" << (key ? key->get_data() : "[null]");
        } else {
    ...
    if (result) {
        SYLAR_LOG_ERROR(g_logger) << "OnGetCb result=" << result << " key=" << ctx->key
                                  << " sn=" << ctx->sn
...
void Tair::OnRemoveCb(int result, void* args) {
    ...
    if (result) {
        SYLAR_LOG_ERROR(g_logger) << "OnRemoveCb result=" << result << " key=" << ctx->key
                                  << " sn=" << ctx->sn;
    }
...
void Tair::OnPutCb(int result, void* args) {
    ...
    if (result) {
        SYLAR_LOG_ERROR(g_logger) << "OnPutCb result=" << result << " key=" << ctx->key
                                  << " sn=" << ctx->sn
                                  << " used=" << (sylar::GetCurrentMS() - ctx->start);
    }
...
void Tair::onTimer(Ctx::ptr ctx) {
    SYLAR_LOG_ERROR(g_logger) << "onTimer sn=" << ctx->sn << " key=" << ctx->key
                              << " used=" << (sylar::GetCurrentMS() - ctx->start);
    sylar::Mutex::Lock lock(m_mutex);
    if (ctx->fiber) {
...
std::string Tair::Ctx::toString() const {
    std::stringstream ss;
    ss << "[ctx"
       << " result=" << result << " key=" << key << " val=" << val << " sn=" << sn << "]";
    return ss.str();
}

TairManager::TairManager() : m_idx(0) {
    init();
}
...
int32_t TairUtil::Get(const std::string& name, const std::string& key, std::string& val, int area,
                      int timeout_ms) {
...
int32_t TairUtil::Put(const std::string& name, const std::string& key, const std::string& val,
                      int area, int expired, int version) {
    auto tair = TairMgr::GetInstance()->get(name);
    if (!tair) {
        return TAIR_RETURN_INVAL_CONN_ERROR;
    }
...
int32_t TairUtil::TryGet(const std::string& name, uint32_t try_count, const std::string& key,
                         std::string& val, int area, int timeout_ms) {
    int32_t rt = -1;
...
int32_t TairUtil::TryPut(const std::string& name, uint32_t try_count, const std::string& key,
                         const std::string& val, int area, int expired, int version) {
    int32_t rt = -1;
    for (uint32_t i = 0; i < try_count; ++i) {
...
int32_t TairUtil::TryRemove(const std::string& name, uint32_t try_count, const std::string& key,
                            int area) {
    int32_t rt = -1;
    for (uint32_t i = 0; i < try_count; ++i) {
