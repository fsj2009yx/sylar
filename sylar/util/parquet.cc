...
ArrowListArray::ArrowListArray(std::shared_ptr<arrow::ListArray> data) : m_data(data) {}
...
ArrowChunkedArray::ArrowChunkedArray(std::shared_ptr<arrow::ChunkedArray> data) : m_data(data) {}
...
ArrowSchema::ArrowSchema(std::shared_ptr<arrow::Schema> schema) : m_data(schema) {}
...
ArrowTable::ArrowTable(std::shared_ptr<arrow::Table> tab) : m_data(tab) {}
...
ParquetFileReader::ptr ParquetFileReader::Open(const std::string& filename) {
    try {
        ...
        SYLAR_LOG_ERROR(g_logger) << "ParquetFileReader Open " << filename << " fail, " << e.what();
    }
    return nullptr;
}
...
std::string ParquetFileReader::toString() const {
    ...
    ss << "num_cols=" << meta->num_columns()
       << " num_schema_elements=" << meta->num_schema_elements() << " num_rows=" << meta->num_rows()
       << " num_row_groups=" << meta->num_row_groups() << " version=" << meta->version()
       << " created_by="
       << meta->created_by()
...
int32_t ArrowTableBuilder::addColumn(const std::string& name, std::shared_ptr<arrow::DataType> type,
                                     bool nullable) {
    auto idx = getColumnIndex(name);
    if (idx != -1) {
        return -1;
...
#define XX(m, b)                                                                      \
    case arrow::Type::m:                                                              \
        m_builders.push_back(                                                         \
            std::make_shared<arrow::b##Builder>(type, arrow::default_memory_pool())); \
        break;
        XX(NA, Null);
        XX(BOOL, Boolean);
        XX(UINT8, UInt8);
...
int32_t ArrowTableBuilder::addListColumn(const std::string& name,
                                         std::shared_ptr<arrow::DataType> type, bool list_nullable,
                                         bool value_nullable) {
    auto idx = getColumnIndex(name);
    if (idx != -1) {
        return -1;
...
#define XX(m, b)                                                                     \
    case arrow::Type::m:                                                             \
        m_builders.push_back(std::make_shared<arrow::ListBuilder>(                   \
...
int32_t ArrowTableBuilder::addMapColumn(const std::string& name,
...
#define XX(m, b)                                                                           \
    case arrow::Type::m:                                                                   \
        val_builder =                                                                      \
            std::make_shared<arrow::b##Builder>(value_type, arrow::default_memory_pool()); \
        break;
        TYPE_LIST(XX)
#undef XX
        default:
    ...
    m_builders.push_back(std::make_shared<arrow::MapBuilder>(arrow::default_memory_pool(),
                                                             key_builder, val_builder, key_sorted));
    return m_fields.size() - 1;
}
