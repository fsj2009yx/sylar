...
class HelloServiceHello
   ...
   public:
    HelloServiceHello() : Base("HelloServiceHello") {}
    int32_t handle(ReqPtr req, RspPtr rsp) override {
        SYLAR_LOG_INFO(g_logger) << "HelloServiceHello req= " << sylar::PBToJsonString(*req)
                                 ...
                                 << " session=" << getSession() << " stream=" << getStream();
        ...
        while (true) {
            ...
            if (req) {
                SYLAR_LOG_INFO(g_logger)
                    << "HelloServiceHelloStreamA req=" << sylar::PBToJsonString(*req);
            } else {
   ...
   public:
    int32_t handle(ServerStream::ptr stream) {
        ...
        while (true) {
            ...
            if (req) {
                SYLAR_LOG_INFO(g_logger)
                    << "HelloServiceHelloStreamC req=" << sylar::PBToJsonString(*req);
            } else {
...
void run() {
    ...
    server->addGrpcServlet(
        sylar::grpc::GrpcServlet::GetGrpcPath("test", "HelloService", "HelloStreamA"),
        std::make_shared<HelloServiceHelloStreamA>());
    server->addGrpcServlet(
        sylar::grpc::GrpcServlet::GetGrpcPath("test", "HelloService", "HelloStreamB"),
        std::make_shared<HelloServiceHelloStreamB>());

    server->addGrpcServlet(
        sylar::grpc::GrpcServlet::GetGrpcPath("test", "HelloService", "HelloStreamC"),
