...
static MYSQL* mysql_init(std::map<std::string, std::string>& params, const int& timeout) {
    ...
    if (mysql_real_connect(mysql, host.c_str(), user.c_str(), passwd.c_str(), dbname.c_str(), port,
                           NULL, 0) == nullptr) {
        SYLAR_LOG_ERROR(g_logger) << "mysql_real_connect(" << host << ", " << port << ", " << dbname
...
static MYSQL_RES* my_mysql_query(MYSQL* mysql, const char* sql) {
    ...
    if (::mysql_query(mysql, sql)) {
        SYLAR_LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:" << mysql_error(mysql);
        return nullptr;
    }

    MYSQL_RES* res = mysql_store_result(mysql);
    if (res == nullptr) {
        SYLAR_LOG_ERROR(g_logger) << "mysql_store_result() error:" << mysql_error(mysql);
    }
    return res;
}
...
MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string& stmt) {
    ...
    if (mysql_stmt_prepare(st, stmt.c_str(), stmt.size())) {
        SYLAR_LOG_ERROR(g_logger) << "stmt=" << stmt << " errno=" << mysql_stmt_errno(st)
                                  << " errstr=" << mysql_stmt_error(st);
        mysql_stmt_close(st);
        return nullptr;
    }
    int count = mysql_stmt_param_count(st);
    MySQLStmt::ptr rt = sylar::protected_make_shared<MySQLStmt>(db, st);
...
MySQLStmtRes::ptr MySQLStmtRes::Create(std::shared_ptr<MySQLStmt> stmt) {
    ...
    MySQLStmtRes::ptr rt = sylar::protected_make_shared<MySQLStmtRes>(stmt, eno, errstr);
    if (eno) {
        return rt;
    }
    MYSQL_RES* res = mysql_stmt_result_metadata(stmt->getRaw());
    if (!res) {
        return sylar::protected_make_shared<MySQLStmtRes>(stmt, stmt->getErrno(),
                                                          stmt->getErrStr());
    }
...
MySQLTransaction::ptr MySQLTransaction::Create(MySQL::ptr mysql, bool auto_commit) {
    MySQLTransaction::ptr rt = sylar::protected_make_shared<MySQLTransaction>(mysql, auto_commit);
    if (rt->begin()) {
        return rt;
    }
    return nullptr;
...
int MySQLManager::execute(const std::string& name, const std::string& sql) {
    auto conn = get(name);
    if (!conn) {
        SYLAR_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name << ") fail, sql=" << sql;
        return -1;
    }
...
ISQLData::ptr MySQLManager::query(const std::string& name, const std::string& sql) {
    auto conn = get(name);
    if (!conn) {
        SYLAR_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name << ") fail, sql=" << sql;
        return nullptr;
    }
...
MySQLTransaction::ptr MySQLManager::openTransaction(const std::string& name, bool auto_commit) {
    auto conn = get(name);
    if (!conn) {
        SYLAR_LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name << ") fail";
        return nullptr;
    }
