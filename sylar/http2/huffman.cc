...
int hf_string_encode(const char *buff_in, int size, int prefix, unsigned char *buff_out,
                     int *size_out);
...
int hf_string_encode(const char *buff_in, int size, int prefix, unsigned char *buff_out,
    ...
    int i = 0;
    int remain = (8 - prefix);
    int j = 0;  // j is instead currently index of buff_out and it is size of buff_out after it has
                // been done.
    int nbytes = 0;
