...
FileInfo::FileInfo() : m_mtime(0), m_size(0), m_loadTime(0) {}
...
std::string FileInfo::toString() const {
    std::stringstream ss;
    ss << "[FileInfo path=" << m_path << " md5=" << m_md5 << " data_size=" << m_size
       << " mtime=" << sylar::Time2Str(m_mtime) << " load_time=" << sylar::Time2Str(m_loadTime)
       << "]";
    return ss.str();
}
...
FileInfo::ptr FileInfoManager::load(const std::string& path, uint64_t limit) {
    ...
    do {
        ...
        if (!sylar::ReadFixFromStreamWithSpeed(ifs, &new_info->m_data[0], new_info->m_data.size(),
                                               limit)) {
            SYLAR_LOG_ERROR(g_logger)
                << path << " ReadFixFromStreamWithSpeed error: " << strerror(errno);
            break;
        }
        std::string md5sum = sylar::md5(new_info->m_data);
        if (md5sum != md5_value) {
            SYLAR_LOG_ERROR(g_logger)
                << "filename=" << path << " md5=" << md5sum << " not (" << md5_value << ")";
            break;
        }
