...
IPAddress::ptr Address::LookupAnyIPAddress(const std::string& host, int family, int type,
                                           int protocol) {
    std::vector<Address::ptr> result;
    if (Lookup(result, host, family, type, protocol)) {
        // for(auto& i : result) {
...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host, int family,
    ...
    if (error) {
        SYLAR_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", " << family << ", "
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if (result <= 0) {
        SYLAR_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", " << port
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if (result <= 0) {
        SYLAR_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", " << port
                                  << ") rt=" << result << " errno=" << errno
                                  << " errstr=" << strerror(errno);
        return nullptr;
    }
