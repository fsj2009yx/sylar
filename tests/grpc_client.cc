    ...
    for (int x = 0; x < 20; ++x) {
        sylar::IOManager::GetThis()->schedule([conn, x, prefx]() {
            ...
            for (int i = 0; i < 20; ++i) {
                ...
                if (rsp->getResponse()) {
                    ...
                    if (hrp) {
                        ...
                        SYLAR_LOG_INFO(g_logger) << "========" << rsp->getResult() << " - "
                                                 << sylar::PBToJsonString(*hr);
                    } else {
                        SYLAR_LOG_INFO(g_logger) << "########" << rsp->getResult() << " - "
                                                 << sylar::PBToJsonString(*hr);
                        ++error;
            ...
            SYLAR_LOG_INFO(g_logger)
                << "=======fail=" << fail << " error=" << error << " =========" << std::endl;
        });
    }
}
...
void testStreamClient(sylar::grpc::GrpcConnection::ptr conn) {
    while (true) {
        for (int x = 0; x < 100; ++x) {
            ...
            SYLAR_LOG_INFO(g_logger) << "===== HelloStreamA - " << rsp << " - "
                                     << (rsp ? sylar::PBToJsonString(*rsp) : "(null)");
            // sleep(1);
        }
        sleep(12);
    }
...
void testStreamClient2(sylar::grpc::GrpcConnection::ptr conn) {
    while (true) {
        for (int x = 0; x < 100; ++x) {
            auto stm = conn->openGrpcClientStream<test::HelloRequest, test::HelloResponse>(
                "/test.HelloService/HelloStreamA");
            // sleep(1);
            for (int i = 0; i < 10; ++i) {
                auto hr = std::make_shared<test::HelloRequest>();
                hr->set_id("hello_stream_a");
            ...
            SYLAR_LOG_INFO(g_logger) << "===== testStreamClient2 HelloStreamA - " << rsp << " - "
                                     << (rsp ? sylar::PBToJsonString(*rsp) : "(null)");
            // sleep(1);
        }
        sleep(12);
    ...
    while (true) {
        for (int x = 0; x < 100; ++x) {
            ...
            while (true) {
                auto rsp = stm->recvMessage<test::HelloResponse>();
                if (rsp) {
                    SYLAR_LOG_INFO(g_logger) << "===== HelloStreamB - " << rsp << " - "
                                             << (rsp ? sylar::PBToJsonString(*rsp) : "(null)");
                } else {
...
void testStreamServer2(sylar::grpc::GrpcConnection::ptr conn) {
    while (true) {
        for (int x = 0; x < 100; ++x) {
            ...
            hr->set_id("hello_stream_b");
            hr->set_msg("world_stream_b");
            auto stm = conn->openGrpcServerStream<test::HelloRequest, test::HelloResponse>(
                "/test.HelloService/HelloStreamB", hr);
            while (true) {
                auto rsp = stm->recv();
                if (rsp) {
                    SYLAR_LOG_INFO(g_logger)
                        << "testStreamServer2 ===== HelloStreamB - " << rsp << " - "
                        << (rsp ? sylar::PBToJsonString(*rsp) : "(null)");
                } else {
    ...
    while (true) {
        for (int x = 0; x < 100; ++x) {
            ...
            while (true) {
                auto rsp = stm->recvMessage<test::HelloResponse>();
                if (rsp) {
                    SYLAR_LOG_INFO(g_logger) << "===== HelloStreamC - " << rsp << " - "
                                             << (rsp ? sylar::PBToJsonString(*rsp) : "(null)");
                } else {
...
void testStreamBidi2(sylar::grpc::GrpcConnection::ptr conn) {
    while (true) {
        for (int x = 0; x < 100; ++x) {
            auto stm = conn->openGrpcBidirectionStream<test::HelloRequest, test::HelloResponse>(
                "/test.HelloService/HelloStreamC");
            auto wg = sylar::WorkerGroup::Create(1);
            wg->schedule([stm]() {
                for (int i = 0; i < 10; ++i) {
                    auto hr = std::make_shared<test::HelloRequest>();
            ...
            while (true) {
                ...
                if (rsp) {
                    SYLAR_LOG_INFO(g_logger)
                        << "testStreamBidi2 ===== HelloStreamC - " << rsp << " - "
                        << (rsp ? sylar::PBToJsonString(*rsp) : "(null)");
                } else {
