...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    do {
        ...
        if (len <= 0) {
            close();
            // SYLAR_LOG_ERROR(g_logger) << "recv_response len <= 0 errno=" << errno << " errstr="
            // << strerror(errno)
            //     << " [" << all_data << "]";
            return nullptr;
        }
        ...
        if (parser->hasError()) {
            close();
            // SYLAR_LOG_ERROR(g_logger) << "recv_response parser has error errno=" << errno << "
        ...
        if (offset == (int)buff_size) {
            close();
            // SYLAR_LOG_ERROR(g_logger) << "recv_response offset == buff_size errno=" << errno << "
            // errstr=" << strerror(errno)
            //     << " [" << all_data << "]";
            return nullptr;
        }
        ...
        do {
            ...
            do {
                if (!begin || len == 0) {
                    ...
                    if (rt <= 0) {
                        close();
                        // SYLAR_LOG_ERROR(g_logger) << "recv_response rt <= 0 " << rt << " errno="
                ...
                if (len == (int)buff_size) {
                    close();
                    // SYLAR_LOG_ERROR(g_logger) << "recv_response len == buff_size errno=" << errno
                    // << " errstr=" << strerror(errno)
                    //     << " [" << all_data << "]";
                    return nullptr;
                }
        ...
        if (length > 0) {
            ...
            if (length > 0) {
                if (readFixSize(&body[len], length) <= 0) {
                    close();
                    // SYLAR_LOG_ERROR(g_logger) << "recv_response readFixSize error errno=" <<
                    // errno << " errstr=" << strerror(errno)
                    //         << " [" << all_data << "]";
                    return nullptr;
                }
