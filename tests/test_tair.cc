...
void do_insert(uint32_t step, uint32_t batch) {
    ...
    for (size_t i = 0; i < 100000000; ++i) {
        int rt = s_tair->put("ytest_" + std::to_string(i * batch + step),
                             s_strings[i % s_strings.size()], 1, 3600 * 24 * 2);
        if (rt) {
    ...
    for (int i = 0; i < 100000000; ++i) {
        ...
        if (rt) {
            SYLAR_LOG_ERROR(g_logger)
                << "error i=" << i << " rt=" << rt << " fibers=" << sylar::Fiber::TotalFibers();
        } else {
...
void run() {
    std::map<std::string, std::string> conf = {{"master_addr", "192.168.106.217:15198"},
    ...
    sylar::IOManager::GetThis()->addTimer(
        ...
        []() {
            ...
            SYLAR_LOG_INFO(g_logger)
                << "rows=" << (tmp - s_total2) << " used=" << (s_ts2 - ts) / 1000.0 << "s "
                << " qps=" << (tmp - s_total2) / ((s_ts2 - ts) / 1000.0)
                << " fibers=" << sylar::Fiber::TotalFibers() << " s_error=" << s_error
                << " s_total3=" << s_total3 << " err_rate=" << (s_error * 100.0 / s_total3);
