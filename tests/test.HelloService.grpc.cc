...
void RegisterService(sylar::grpc::GrpcServer::ptr server) {
    ...
    server->addGrpcServlet("/test.HelloService/HelloStreamA",
                           std::make_shared<HelloStreamAServlet>());
    SYLAR_LOG_INFO(g_logger) << "register grpc /test.HelloService/HelloStreamA";
    server->addGrpcServlet("/test.HelloService/HelloStreamB",
                           std::make_shared<HelloStreamBServlet>());
    SYLAR_LOG_INFO(g_logger) << "register grpc /test.HelloService/HelloStreamB";
    server->addGrpcServlet("/test.HelloService/HelloStreamC",
                           std::make_shared<HelloStreamCServlet>());
