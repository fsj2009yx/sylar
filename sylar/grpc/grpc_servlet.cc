...
std::string GrpcServlet::GetGrpcPath(const std::string& ns, const std::string& service,
...
GrpcServlet::GrpcServlet(const std::string& name, GrpcType type)
    : sylar::http::Servlet(name), m_type(type) {}
...
int32_t GrpcServlet::processStream(sylar::grpc::GrpcRequest::ptr request,
                                   ...
                                   sylar::grpc::GrpcSession::ptr session) {
    SYLAR_LOG_WARN(g_logger) << "GrpcServlet name=" << m_name
                             << " unhandle stream processStream type=" << (uint32_t)m_type;
    session->sendRstStream(request->getRequest()->getStreamId(), 404);
    return -1;
}
...
int32_t GrpcServlet::handle(sylar::http::HttpRequest::ptr request,
    ...
    if (content_type.empty() || content_type.find("application/grpc") == std::string::npos) {
    ...
    sylar::grpc::GrpcSession::ptr h2session =
        std::dynamic_pointer_cast<sylar::grpc::GrpcSession>(session);
...
GrpcFunctionServlet::GrpcFunctionServlet(GrpcType type, callback cb, stream_callback scb)
    : GrpcServlet("GrpcFunctionServlet", type), m_cb(cb), m_scb(scb) {}
...
int32_t GrpcFunctionServlet::process(sylar::grpc::GrpcRequest::ptr request,
    ...
    if (m_cb) {
        ...
        session->sendRstStream(request->getRequest()->getStreamId(), 404);
        SYLAR_LOG_WARN(g_logger) << "GrpcFunctionServlet unhandle process "
...
int32_t GrpcFunctionServlet::processStream(sylar::grpc::GrpcRequest::ptr request,
    ...
    if (m_scb) {
        ...
        session->sendRstStream(request->getRequest()->getStreamId(), 404);
        SYLAR_LOG_WARN(g_logger) << "GrpcFunctionServlet unhandle processStream "
                                 << request->getRequest()->getPath();
        return -1;
    }
}

GrpcFunctionServlet::ptr GrpcFunctionServlet::Create(GrpcType type, callback cb,
                                                     stream_callback scb) {
