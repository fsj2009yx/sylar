...
void test_batch(sylar::SQLite3::ptr db) {
    ...
    trans.begin();
    sylar::SQLite3Stmt::ptr stmt =
        sylar::SQLite3Stmt::Create(db, "insert into user(name, age) values(?, ?)");
    for (int i = 0; i < n; ++i) {
        stmt->reset();
...
int main(int argc, char** argv) {
    ...
    if (!db) {
        ...
        db = sylar::SQLite3::Create(dbname, sylar::SQLite3::READWRITE | sylar::SQLite3::CREATE);
        if (!db) {
...
#undef XX
        ...
        if (rt != SQLITE_OK) {
            SYLAR_LOG_ERROR(g_logger)
                << "create table error " << db->getErrno() << " - " << db->getErrStr();
            return 0;
        }
    }
    ...
    for (int i = 0; i < 10; ++i) {
        if (db->execute("insert into user(name, age) values(\"name_%d\",%d)", i, i) != SQLITE_OK) {
            SYLAR_LOG_ERROR(g_logger)
                << "insert into error " << i << " " << db->getErrno() << " - " << db->getErrStr();
        }
    }

    sylar::SQLite3Stmt::ptr stmt =
        sylar::SQLite3Stmt::Create(db, "insert into user(name, age, create_time) values(?, ?, ?)");
    if (!stmt) {
