...
std::string ReplaceSuffix(const std::string& s, const std::string& search,
...
void print_include(std::vector<std::string> headers, google::protobuf::io::Printer& printer) {
...
void print_include_macro(std::string filename, google::protobuf::io::Printer& printer) {
...
bool generate_method_header(const google::protobuf::FileDescriptor* file,
                            ...
                            std::string* error, const google::protobuf::ServiceDescriptor* svc,
    ...
    std::vector<std::string> headers = {"\"sylar/grpc/grpc_servlet.h\"", "\"" + pb_filename + "\""};
    ...
    if (mtd->client_streaming() && mtd->server_streaming()) {
        printer.Print(
            "class $item$ : public sylar::grpc::GrpcStreamBidirectionServlet<$input$, $output$> "
            "{\n",
            "item", servlet_name, "input", input_class_name, "output", output_class_name);
        printer.Print("public:\n");
        printer.Print("\tint32_t handle(ServerStream::ptr stream) override;\n");
    } else if (mtd->client_streaming()) {
        printer.Print(
            "class $item$ : public sylar::grpc::GrpcStreamClientServlet<$input$, $output$> {\n",
            "item", servlet_name, "input", input_class_name, "output", output_class_name);
        printer.Print("public:\n");
        printer.Print("\tint32_t handle(ServerStream::ptr stream, RspPtr rsp) override;\n");
    } else if (mtd->server_streaming()) {
        printer.Print(
            "class $item$ : public sylar::grpc::GrpcStreamServerServlet<$input$, $output$> {\n",
            "item", servlet_name, "input", input_class_name, "output", output_class_name);
        printer.Print("public:\n");
    ...
    printer.Print("\tGRPC_SERVLET_INIT_NAME($item$, \"$name$\");\n\n", "item", servlet_name, "name",
                  ToGrpcPath(mtd->full_name()));
...
bool generate_method_source(const google::protobuf::FileDescriptor* file,
    ...
    if (mtd->client_streaming() && mtd->server_streaming()) {
        ...
        printer.Print("int32_t $item$::handle(ServerStream::ptr stream, ReqPtr req) {\n", "item",
                      servlet_name);
        printer.Print(
            "\tSYLAR_LOG_WARN(g_logger) << \"Unhandle $item$ req=\" << "
...
bool generate_method(const google::protobuf::FileDescriptor* file, const std::string& parameter,
...
bool generate_service_header(const google::protobuf::FileDescriptor* file,
    ...
    std::vector<std::string> headers = {"\"sylar/grpc/grpc_server.h\""};
    print_include(headers, printer);
...
bool generate_service_source(const google::protobuf::FileDescriptor* file,
    ...
    std::vector<std::string> headers = {"\"" + output_path + "." + svc->name() + ".grpc.h" + "\"",
                                        "\"sylar/log.h\""};

    for (int i = 0; i < svc->method_count(); ++i) {
        auto m = svc->method(i);
        headers.push_back("\"" + output_path + "." + svc->name() + "." + m->name() + ".grpc.h" +
                          "\"");
    }
    ...
    for (int i = 0; i < svc->method_count(); ++i) {
        ...
        printer.Print("\tserver->addGrpcServlet(\"$path$\", std::make_shared<$clazz$>());\n",
                      "path", ToGrpcPath(m->full_name()), "clazz", m->name() + "Servlet");
        printer.Print("\tSYLAR_LOG_INFO(g_logger) << \"register grpc $path$\";\n", "path",
                      ToGrpcPath(m->full_name()));
    }
...
bool generate_service(const google::protobuf::FileDescriptor* file, const std::string& parameter,
                      google::protobuf::compiler::GeneratorContext* context, std::string* error,
                      const google::protobuf::ServiceDescriptor* svc) {
    for (int i = 0; i < svc->method_count(); ++i) {
