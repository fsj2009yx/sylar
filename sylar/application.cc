...
static sylar::ConfigVar<std::string>::ptr g_service_discovery_redis = sylar::Config::Lookup(
...
static sylar::ConfigVar<std::string>::ptr g_service_discovery_consul =
    sylar::Config::Lookup("service_discovery.consul", std::string(""), "service discovery consul");

SYLAR_DEFINE_CONFIG(sylar::ConsulRegisterInfo::ptr, g_consul_register_info, "consul.register_info",
...
int Application::run_fiber() {
    ...
    for (auto& i : http_confs) {
        ...
        if (i.type == "http") {
            server = std::make_shared<sylar::http::HttpServer>(i.keepalive, process_worker,
            ...
            server =
                std::make_shared<sylar::grpc::GrpcServer>(process_worker, io_worker, accept_worker);
        } else if (i.type == "ws") {
            server =
                std::make_shared<sylar::http::WSServer>(process_worker, io_worker, accept_worker);
        } else if (i.type == "rock") {
            server = std::make_shared<sylar::RockServer>("rock", process_worker, io_worker,
                                                         accept_worker);
        } else if (i.type == "nameserver") {
            server = std::make_shared<sylar::RockServer>("nameserver", process_worker, io_worker,
                                                         accept_worker);
    ...
    if (!g_service_discovery_consul->getValue().empty()) {
        if (!g_consul_register_info->getValue()) {
            SYLAR_LOG_ERROR(g_logger)
                << "consul info invalid " << g_consul_register_info->getName();
            return false;
        }
