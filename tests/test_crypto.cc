...
void test_aes256ecb() {
    for (int i = 0; i < 4097; ++i) {
        ...
        int32_t len =
            sylar::CryptoUtil::AES256Ecb(key.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if (len > 0) {
            encode.resize(len);
        }
        len = sylar::CryptoUtil::AES256Ecb(key.c_str(), encode.c_str(), encode.size(), &decode[0],
                                           false);
...
void test_aes256cbc() {
    for (int i = 0; i < 4097; ++i) {
        ...
        int32_t len = sylar::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(), str.c_str(), str.size(),
                                                   &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if (len > 0) {
            encode.resize(len);
        }
        len = sylar::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(), encode.c_str(), encode.size(),
                                           &decode[0], false);
        std::cout << "decode_len: " << len << std::endl;
        if (len > 0) {
            decode.resize(len);
...
void test_one() {
    ...
    int32_t len = sylar::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), str.c_str(), str.size(),
                                               &encode[0], true);
    if (len >= 0) {
        encode.resize(len);
    }

    int32_t len2 = sylar::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), encode.c_str(),
    ...
    std::cout << "encode: " << sylar::base64encode(encode) << " - len=" << encode.size()
              << std::endl;
...

    {
        ...
        encode.resize(str.size() + 30);
        int olen = 0;
        EVP_EncryptUpdate(&ctx, (uint8_t*)&encode[0], &olen, (const uint8_t*)str.c_str(),
                          str.length());
        int llen = 0;
...
void test_rsa() {
    ...
    // std::cout << "generate: " << sylar::RSACipher::GenerateKey(pubkey_path, prikey_path) <<
    // std::endl;
    ...
    std::cout << "rsa pubsize: " << rsa->getPubRSASize() << " prisize: " << rsa->getPriRSASize()
              << std::endl;
