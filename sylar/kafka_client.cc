...
class MyDeliveryReportCb : public RdKafka::DeliveryReportCb {
   public:
    ...
    void dr_cb(RdKafka::Message& message) {
        if (message.err()) {
            SYLAR_LOG_ERROR(g_logger)
                << "Message delivery for (" << message.len() << " bytes): " << message.errstr()
...
void KafkaConsumer::start(const std::string& topic_name, int32_t partition, int64_t start_offset,
    ...
    do {
        ...
        if (resp != RdKafka::ERR_NO_ERROR) {
            SYLAR_LOG_ERROR(g_logger)
                << "Failed to start consumer: " << RdKafka::err2str(resp) << std::endl;
            sleep(2);
        } else {
            break;
        }
...
void KafkaConsumerGroup::start() {
    ...
    for (uint32_t i = m_partitionStart; i < m_partitionEnd; ++i) {
        ...
        sylar::FoxThreadMgr::GetInstance()->dispatch(m_threadName, [this, i, kc]() {
            SYLAR_LOG_INFO(g_logger) << "start consumer topic=" << m_topic << " partition=" << i;
