...
class GrpcSDLoadBalance : public SDLoadBalance {
   public:
    ...
    GrpcResponse::ptr request(const std::string& domain, const std::string& service,
                              const std::string& method, PbMessagePtr message, uint32_t timeout_ms,
    ...
    typename GrpcClientStreamServer<Req, Rsp>::ptr openGrpcServerStream(
        const std::string& domain, const std::string& service, const std::string& method,
        PbMessagePtr message, const std::map<std::string, std::string>& headers = {},
        uint64_t idx = -1) {
    ...
    template <class Req, class Rsp>
    typename GrpcClientStreamClient<Req, Rsp>::ptr openGrpcClientStream(
        const std::string& domain, const std::string& service, const std::string& method,
        const std::map<std::string, std::string>& headers = {}, uint64_t idx = -1) {
        auto conn = getConnAs<GrpcConnection>(domain, service, idx);
        if (!conn) {
            return nullptr;
        }
    ...
    template <class Req, class Rsp>
    typename GrpcClientStreamBidirection<Req, Rsp>::ptr openGrpcBidirectionStream(
        const std::string& domain, const std::string& service, const std::string& method,
        const std::map<std::string, std::string>& headers = {}, uint64_t idx = -1) {
        auto conn = getConnAs<GrpcConnection>(domain, service, idx);
        if (!conn) {
